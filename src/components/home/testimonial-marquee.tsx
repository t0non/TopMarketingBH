  ...
  return (
    <section className="w-full py-20 bg-white relative overflow-hidden">
      ...
      <div className="container mx-auto mb-10 text-center relative z-10">
        <h2 className="text-2xl md:text-3xl font-headline font-extrabold text-foreground mb-2">
          O que dizem quem já testou
        </h2>
        <p className="text-muted-foreground font-medium">
          Novos clientes, novos resultados chegando. Fale com a gente e seja o próximo.
        </p>
      </div>

      <div className="relative w-full z-10">
        {/* Fade Edges */}
        <div className="absolute left-0 top-0 bottom-0 w-24 md:w-40 bg-gradient-to-r from-white via-white/80 to-transparent z-20 pointer-events-none" />
        <div className="absolute right-0 top-0 bottom-0 w-24 md:w-40 bg-gradient-to-l from-white via-white/80 to-transparent z-20 pointer-events-none" />
        ...
        <div className="flex w-full overflow-hidden hover:[&_.animate-marquee]:paused">
          <div className="flex animate-marquee min-w-full shrink-0 gap-6 px-3">
            {[...testimonials, ...testimonials, ...testimonials].map((item, i) => (
              <Card 
                key={i} 
                className="w-[320px] md:w-[380px] shrink-0 bg-white rounded-2xl shadow-sm border border-slate-200 hover:border-blue-500/30 hover:shadow-xl hover:shadow-blue-500/5 transition-all duration-300 group overflow-hidden relative"
              >
                {/* Large Watermark Quote */}
                <Quote className="absolute -bottom-4 -right-4 w-32 h-32 text-slate-100 rotate-12 z-0" />
                
                <CardContent className="p-6 relative z-10">
                  <div className="flex justify-between items-start mb-5">
                    {/* Avatar & Name */}
                    <div className="flex items-center gap-3">
                      <div className="relative h-12 w-12 rounded-full overflow-hidden border-2 border-slate-100 shadow-sm group-hover:scale-105 transition-transform duration-300">
                         <Image 
                           src={item.image} 
                           alt={item.name}
                           fill
